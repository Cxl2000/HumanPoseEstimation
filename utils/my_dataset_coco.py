...
class CocoKeypoint(data.Dataset):
    def __init__(self,
        ...
        self.anno_path = os.path.join(root, "annotations_trainval2017", anno_file)
        ...
        for img_id in img_ids:
            ...
            ann_ids = det.getAnnIds(imgIds=img_id)  # 获取该图片所有标注ID
            anns = det.loadAnns(ann_ids)    #获取该标注ID的所有标注信息

            # 一张图片可能有多个标注信息，遍历每个标注信息
            for ann in anns:
    ...
    def __getitem__(self, idx):
        ...
        """
            image:  H W C格式的RGB图像
            target: dict格式{"image_path图片绝对路径",
                             "image_width图片宽",
                             "image_height图片高",
                             "keypoints关键点信息(17,2)",
                             "visible关键点可见度(17,1)"}
